...
function _renderRecomendacion(primary, secondary) {
  ...
  if (ctaMain && primary) {
    ctaMain.href = `/store/index.html#${primary.id}`;
  }
}
...
function openPerfumeModal(perfumeId) {
  ...
  if (ctaEl) {
    ...
    ctaEl.href = `/store/index.html#${perfumeId}`;
  }
...
function downloadResultPDF() {
  ...
  const { presencia, aiText, primaryPerfume, secondaryPerfume, scores, presenciaKey } = state;
  ...
  const normalizedScores = normalizeScores(scores);
  const topAffinities = Object.keys(normalizedScores)
    .sort((a, b) => normalizedScores[b] - normalizedScores[a])
    .slice(0, 8)
    .map(k => ({
      label: LABELS[k] || k,
      pct: normalizedScores[k],
      isPrimary: k === presenciaKey
    }));

  // Genera el markup compacto de una tarjeta de fragancia (usado en las 2 columnas)
  function _fragranceCard(p, d, lbl, ac) {
    if (!p) return '';
    const extras = [d.concentracion, d.presentacion, d.clima, d.ocasion].filter(Boolean).join(' &middot; ');
    return `
    <div style="background:#fff;border-radius:6px;border:1px solid rgba(191,167,106,0.25);padding:14px 16px;height:100%;box-sizing:border-box;">
      <div style="font-size:6px;letter-spacing:2px;text-transform:uppercase;color:${ac};margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid rgba(191,167,106,0.2);">${lbl}</div>
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          <td style="width:78px;vertical-align:top;">
            ${p.image && p.image !== 'x' && p.image !== '' ? `<img src="${p.image}" style="width:78px;height:98px;object-fit:cover;border-radius:4px;display:block;">` : `<div style="width:78px;height:98px;background:rgba(191,167,106,0.12);border-radius:4px;"></div>`}
          </td>
          <td style="padding-left:12px;vertical-align:top;">
            <div style="font-size:7px;letter-spacing:1px;text-transform:uppercase;color:#7a6a5a;margin-bottom:3px;">${d.familia || '&mdash;'}</div>
            <div style="font-size:13px;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;color:#1a1a1a;margin-bottom:5px;line-height:1.15;">${p.name}</div>
            ${d.rationale ? `<p style="font-size:8px;line-height:1.4;color:#3a2a22;margin:0;font-style:italic;">${d.rationale}</p>` : ''}
          </td>
        </tr>
      </table>
      ${d.notas ? `<p style="font-size:7.5px;color:#7a6a5a;margin:8px 0 0;"><strong style="color:#2a201a;">Notas:</strong> ${d.notas.salida} &middot; ${d.notas.corazon} &middot; ${d.notas.fondo}</p>` : ''}
      ${extras ? `<p style="font-size:7px;color:#7a6a5a;margin:5px 0 0;">${extras}</p>` : ''}
    </div>`;
  }

  const fullPDF_HTML = `
  <div style="font-family:Arial,Helvetica,sans-serif;color:#2a201a;background:#F0E7DA;width:794px;box-sizing:border-box;">

    <div style="background:#1a1a1a;padding:28px 52px 24px;">
      <div style="font-size:7px;letter-spacing:4px;text-transform:uppercase;color:#bfa76a;margin-bottom:8px;">Diagnóstico de Presencia &middot; Praessia</div>
      <div style="font-size:32px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:#F0E7DA;line-height:1;">${presencia.nombre.toUpperCase()}</div>
      <div style="font-size:9px;color:rgba(240,231,218,0.4);margin-top:6px;">Informe personal &middot; Praessia</div>
    </div>

    <div style="background:#6a2831;padding:10px 52px;">
      <p style="font-size:10.5px;color:#F0E7DA;margin:0;font-style:italic;">${presencia.tagline}</p>
    </div>

    <div style="padding:24px 52px 8px;">
      <table style="width:100%;border-collapse:collapse;table-layout:fixed;">
        <tr>
          <td style="width:58%;padding-right:26px;vertical-align:top;border-right:1px solid rgba(191,167,106,0.3);">
            <div style="font-size:7px;letter-spacing:3px;text-transform:uppercase;color:#6a2831;margin-bottom:8px;">Tu interpretación</div>
            <p style="font-size:9.5px;line-height:1.55;color:#2a201a;margin:0 0 14px;font-weight:300;">${txt}</p>
            <div style="font-size:7px;letter-spacing:3px;text-transform:uppercase;color:#6a2831;margin-bottom:8px;">Tu presencia en contexto</div>
            <table style="width:100%;border-collapse:collapse;background:rgba(191,167,106,0.15);">
              ${[['Entorno laboral', ctx.laboral], ['Entorno social', ctx.social], ['Momento íntimo', ctx.intimo], ['Lo que proyectas', ctx.proyeccion]].map(([l, t]) => `
              <tr>
                <td style="padding:7px 12px;border:1px solid rgba(191,167,106,0.2);vertical-align:top;">
                  <div style="font-size:6px;letter-spacing:2px;text-transform:uppercase;color:#6a2831;margin-bottom:3px;">${l}</div>
                  <p style="font-size:8.5px;line-height:1.4;color:#2a201a;margin:0;font-weight:300;">${t}</p>
                </td>
              </tr>`).join('')}
            </table>
          </td>
          <td style="width:42%;padding-left:26px;vertical-align:top;">
            <div style="font-size:7px;letter-spacing:3px;text-transform:uppercase;color:#6a2831;margin-bottom:10px;">Mapa de afinidades</div>
            ${topAffinities.map(a => `
            <div style="margin-bottom:7px;">
              <table style="width:100%;margin-bottom:3px;border-collapse:collapse;">
                <tr>
                  <td style="font-size:7.5px;letter-spacing:0.05em;text-transform:uppercase;color:${a.isPrimary ? '#6a2831' : '#7a6a5a'};font-weight:${a.isPrimary ? '700' : '400'};">${a.label}</td>
                  <td style="text-align:right;font-size:7.5px;color:${a.isPrimary ? '#6a2831' : '#7a6a5a'};font-weight:${a.isPrimary ? '700' : '400'};">${a.pct}%</td>
                </tr>
              </table>
              <div style="height:3px;background:rgba(191,167,106,0.2);border-radius:2px;">
                <div style="height:3px;width:${a.pct}%;background:${a.isPrimary ? '#6a2831' : 'rgba(191,167,106,0.6)'};border-radius:2px;"></div>
              </div>
            </div>`).join('')}
          </td>
        </tr>
    ...
    <div style="background:#1a1a1a;padding:14px 52px;margin-top:8px;">
      <div style="font-size:7px;letter-spacing:4px;text-transform:uppercase;color:#bfa76a;margin-bottom:3px;">Selección de fragancias</div>
      <div style="font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#F0E7DA;">Para la presencia ${presencia.nombre}</div>
    </div>

    <div style="padding:20px 52px 24px;">
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          <td style="width:50%;vertical-align:top;padding-right:10px;">
            ${_fragranceCard(primaryPerfume, pd, 'Fragancia principal', '#6a2831')}
          </td>
          <td style="width:50%;vertical-align:top;padding-left:10px;">
            ${_fragranceCard(secondaryPerfume, sd, 'Segunda propuesta', '#7a6a5a')}
          </td>
        </tr>
      </table>
    </div>
    ...
    <table style="width:100%;background:#1a1a1a;border-collapse:collapse;">
      <tr>
        <td style="font-size:7px;letter-spacing:2px;text-transform:uppercase;color:rgba(240,231,218,0.25);padding:12px 0 12px 52px;">La esencia de estar presente</td>
        <td style="text-align:right;font-size:7px;letter-spacing:2px;text-transform:uppercase;color:rgba(240,231,218,0.25);padding:12px 52px 12px 0;">praessia.com</td>
      </tr>
    </table>
  </div>`;
  ...
  wrapper.style.cssText = 'position:fixed;top:0;left:0;width:794px;opacity:0;pointer-events:none;z-index:9999;';
  wrapper.innerHTML = fullPDF_HTML;
  ...
  setTimeout(() => {
    html2canvas(pageEl, {
      ...
      const imgHeight = canvas.height / (canvas.width / 794);
      const doc = new jsPDF({ unit: 'px', format: [794, imgHeight], orientation: 'portrait' });
      const img = canvas.toDataURL('image/jpeg', 0.97);
      doc.addImage(img, 'JPEG', 0, 0, 794, imgHeight);
      doc.save(`Praessia_${presencia.nombre.replace(/\s+/g, '_')}.pdf`);
